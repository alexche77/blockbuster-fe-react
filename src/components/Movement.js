...
export default function Movement({ movement, readOnly }) {
  ...
  const handleUpdate = () => {
    console.log(
      `Updating movement # ${movement.id} --> Price ${price} / Quantity ${quantity}}`
    );
    updateMovement(movement.id, {
      price: price,
      quantity: quantity,
      unit_price: (price / quantity).toFixed(2),
    })
      .then(({ data }) => {
      ...
      <Card.Body>
        ...
        <Form.Row className="my-3">
          <Form.Group as={Col} controlId="formMovementQuantity">
            ...
            <Form.Control
              disabled={readOnly || movement.order.order_state !== 0}
              type="number"
              min="1"
              step="1"
              value={quantity}
            ...
            {!readOnly && (
              <Form.Text id="unitPriceHelpBlock" muted>
                ...
                <br></br>
                Profit margin: {movement.movie.profit_percentage} %<br></br>
              </Form.Text>
            )}
          </Form.Group>
        ...
        {!readOnly && (
          <>
            <Button
              className="w-100"
              variant="dark"
              disabled={!updatePending || movement.order.order_state !== 0}
            ...
            <Button
              ...
              disabled={movement.order.order_state !== 0}
            >
              Delete
            </Button>
          </>
